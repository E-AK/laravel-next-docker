...
RUN docker-php-ext-enable sockets grpc amqp

RUN usermod -u 1000 www-data && groupmod -g 1000 www-data \
    && usermod -s /bin/bash www-data

COPY ./docker/notification_service/entrypoint.sh /var/www/entrypoint.sh
COPY ./docker/notification_service/supervisord.conf /etc/supervisor/conf.d/supervisord.conf
